...
(function() {
	...
	const startBookmarklet = function($) {
		...
		var setGridGutters = function() {
			const gutter = Origami['o-grid'].getCurrentGutter();
		...
		if ($("#grid-displayer").length) { // Close grid displayer when the bookmarklet is clicked for a second time
			...
			// Frameworks list
			const origamis = [["o4", "Origami Grid v4"], ["so4", "Origami Grid v4 (snappy)"]];
			...
			gridToolsHtml = String(gridToolsHtml);
