...
module.exports.getBaseKarmaConfig = function (opts = {sassIncludePaths: []}) {
	return Promise.all([
		fileHelpers.getComponentName(),
		fileHelpers.getModuleBrands(),
		fileHelpers.readIfExists(path.resolve("main.scss")),
	]).then(values => {
		...
		return {
			...
			scssPreprocessor: {
				options: {
					file: "",
					data: `$system-code: "origami-build-tools";${
						brands.length ? `$o-brand: ${brands[0]};` : ""
					}${mainScssContent}@if mixin-exists('${primaryMixinName}') {@include ${primaryMixinName}();};`,
